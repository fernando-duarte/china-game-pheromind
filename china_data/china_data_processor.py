...
from china_data.utils.processor_capital import calculate_capital_stock, project_capital_stock, smooth_capital_data
...
def main():
    ...
    # Smooth capital stock data to handle outliers and gaps
    logger.info("Smoothing capital stock and investment data")
    try:
        k_smooth = smooth_capital_data(k_proj, window_size=3, outlier_threshold=3.0, interpolate_gaps=True)
        
        # Log statistics about smoothing
        original_k = k_proj.dropna(subset=['K_USD_bn'])
        smoothed_k = k_smooth.dropna(subset=['K_USD_bn'])
        
        if 'K_USD_bn' in k_smooth.columns and not original_k.empty and not smoothed_k.empty:
            # Count years where values changed
            changed_years = []
            for year in original_k['year'].unique():
                orig_val = original_k.loc[original_k['year'] == year, 'K_USD_bn'].values
                smooth_val = smoothed_k.loc[smoothed_k['year'] == year, 'K_USD_bn'].values
                
                if len(orig_val) > 0 and len(smooth_val) > 0 and orig_val[0] != smooth_val[0]:
                    changed_years.append(year)
            
            if changed_years:
                logger.info(f"Smoothing modified capital stock values for {len(changed_years)} years")
                logger.debug(f"Years with modified capital stock: {sorted(changed_years)}")
            else:
                logger.info("Smoothing did not modify any capital stock values")
                
            # Check for filled gaps
            orig_years = set(original_k['year'].unique())
            smooth_years = set(smoothed_k['year'].unique())
            filled_years = smooth_years - orig_years
            
            if filled_years:
                logger.info(f"Smoothing filled gaps for {len(filled_years)} years: {sorted(filled_years)}")
        
        # Replace k_proj with smoothed version
        k_proj = k_smooth
    except Exception as e:
        logger.warning(f"Error during capital data smoothing: {e}")
        logger.info("Continuing with unsmoothed capital data")

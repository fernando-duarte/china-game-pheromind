...
4. **Extrapolation to {{ end_year }}**:
   ...
   - **ARIMA(1,1,1) model**: {% for var in extrapolation_methods['ARIMA(1,1,1)'] %}
     - {{ var }}{% endfor %}
   - **Average growth rate of historical data**: {% for var in extrapolation_methods['Average growth rate'] %}
     - {{ var }}{% endfor %}
   - **Linear regression**: {% for var in extrapolation_methods['Linear regression'] %}
     - {{ var }}{% endfor %}{% if extrapolation_methods['IMF projections'] %}
   - **IMF projections**: {% for var in extrapolation_methods['IMF projections'] %}
     - {{ var }}: Projected using official IMF Fiscal Monitor projections{% endfor %}{% endif %}{% if extrapolation_methods['Investment-based projection'] %}
   - **Investment-based projection**: {% for var in extrapolation_methods['Investment-based projection'] %}
     - {{ var }}: Projected using the formula K_t = K_{t-1} * (1-delta) + I_t, where delta = 0.05 (5% depreciation rate) and I_t is investment in year t{% endfor %}{% endif %}{% if extrapolation_methods['Extrapolated'] %}
   - **Other methods**: {% for var in extrapolation_methods['Extrapolated'] %}
     - {{ var }}{% endfor %}{% endif %}
